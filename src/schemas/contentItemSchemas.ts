...
// Reference to the content type (or snippet) element this value is for
const elementReferenceSchema = writeReferenceObjectSchema.describe(
  "Reference to the content type element this value is for.",
);

// Type definitions for recursive structures
type Reference = { id?: string; external_id?: string };
...
const urlSlugInVariantElementSchema = z
  .object({
    ...
    mode: z
      ...
      .describe(
        "REQUIRED on every URL slug element. 'autogenerated' (system-generated based on dependent text element) or 'custom' (manual). Switches to custom when value is directly modified.",
      ),
    value: z
      .string()
      .nullable()
      .describe(
        "URL-friendly slug value for SEO. Always send it together with 'mode' — a URL slug element sent without 'mode' is rejected by the Management API.",
      ),
  })
  .describe(
