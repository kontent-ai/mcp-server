...
- **Keep a human in the loop.** Every tool carries MCP annotations (reads are `readOnlyHint`, mutating tools `destructiveHint`) that compliant clients use to auto-approve reads and prompt before destructive calls. Run the server with such a client and avoid headless auto-approve setups against a write-capable key.
