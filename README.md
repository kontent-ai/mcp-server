...
Like any MCP server that returns user-authored content, this server is exposed to **indirect prompt injection**: text stored in your CMS (for example, a content item element written by a Content Editor) can contain instructions that a connected LLM interprets as commands, potentially steering an agent into calling destructive tools (delete / unpublish / overwrite) or leaking unpublished drafts.

This is an industry-wide, **unsolved** problem, and — importantly — **the MCP server cannot reliably fix it by transforming the content it returns.** A model cannot dependably separate "data" from "instructions" in its context, and text-wrapping defenses (delimiters, "treat the following as data" banners) are defeated by an attacker who knows the format — which, for an open-source server, they do (see [Zhan et al., NAACL 2025](https://arxiv.org/abs/2503.00061) and Simon Willison's [lethal trifecta](https://simonwillison.net/2025/Jun/16/the-lethal-trifecta/)). The MCP specification reflects this division of responsibility: the server exposes *hints*, while the **client and a human in the loop** own the decision to permit a tool call ([MCP tools spec, Security](https://modelcontextprotocol.io/specification/2025-06-18/server/tools#security)).

The controls below are therefore layered by how much you can rely on them — deterministic controls first, probabilistic ones last. They apply on both transports (STDIO and the multi-tenant Streamable HTTP product).

**1. Least-privilege Management API key — deterministic, strongest.** The server acts with whatever Management API key it is given. Provision the **most restrictive key the workflow needs**: with a read-only key, a hijacked agent's `delete-*` / `publish-*` call simply fails at the API boundary no matter what any injected text says. This is the only control that holds regardless of model behaviour, and the most important one for the multi-tenant transport. Create scoped keys in the Kontent.ai app (Environment settings → API keys).

**2. Human-in-the-loop client + tool annotations — hint.** Every tool declares [MCP tool annotations](https://modelcontextprotocol.io/specification/2025-06-18/server/tools#tool-annotations): read tools are `readOnlyHint: true` and every mutating tool (including `publish-content-item-variant`) is `destructiveHint: true`. Per the spec these are **hints a client may honour, not server-enforced controls** ("clients MUST consider tool annotations to be untrusted unless they come from trusted servers"). A compliant client (Claude Desktop, VS Code Copilot, …) uses them to auto-approve reads and prompt before destructive calls. Run the server with a client that keeps a human in the loop for destructive actions; avoid fully headless / auto-approve-everything setups against a write-capable key.

**3. Claude Code hooks — deterministic client-side gate, opt-in.** If you connect this server via [Claude Code](https://code.claude.com), you can add a local hook that prompts you before any destructive Kontent.ai tool runs, independent of what the model decided. We ship a ready-to-copy example — see [`docs/security-hooks.md`](docs/security-hooks.md). Hooks are configured locally by you; a server cannot enforce them.

**4. Injection-aware model — defense-in-depth, not sufficient.** Capable models often *detect* injected instructions and surface them instead of obeying. This is valuable but probabilistic — even state-of-the-art defenses do not reach 100% ([Anthropic, Prompt injection defenses](https://www.anthropic.com/research/prompt-injection-defenses)) — so treat it as a layer, never the only control.

> No combination of the above "solves" prompt injection. The robust posture is the least-privilege key (#1) plus a human in the loop (#2/#3), and not pointing a write-capable agent at untrusted content when it also has an exfiltration path. Report security issues privately to security@kontent.ai.
